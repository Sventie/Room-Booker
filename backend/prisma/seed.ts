...
const hubRooms = [
  { roomNumber: '4.12', name: 'Forum',     floor: 4, wing: 'HUB', type: 'MEETING_ROOM', deskCount: 0 },
  { roomNumber: '4.16', name: null,        floor: 4, wing: 'HUB', type: 'DESK_ROOM',    deskCount: 2 },
  { roomNumber: '4.17', name: null,        floor: 4, wing: 'HUB', type: 'DESK_ROOM',    deskCount: 8 },
  { roomNumber: '4.18', name: null,        floor: 4, wing: 'HUB', type: 'DESK_ROOM',    deskCount: 5 },
  { roomNumber: '4.20', name: 'Bellevue',  floor: 4, wing: 'HUB', type: 'MEETING_ROOM', deskCount: 0 },
  { roomNumber: '4.22', name: 'Seegarten', floor: 4, wing: 'HUB', type: 'MEETING_ROOM', deskCount: 0 },
  { roomNumber: '4.23', name: null,        floor: 4, wing: 'HUB', type: 'DESK_ROOM',    deskCount: 8 },
]
...
async function main() {
  ...
  for (const room of wingRooms) {
    ...
    const created = await prisma.room.upsert({
      ...
      update: {},
      create: { roomNumber: room.roomNumber, floor: room.floor, wing: room.wing, deskCount: room.deskCount, svgId,
        desks: { create: Array.from({ length: room.deskCount }, (_, i) => ({ number: i + 1 })) }
      }
    })
    console.log(`  ✓ ${created.roomNumber}`)
  }
  ...
  for (const room of hubRooms) {
    ...
    await prisma.room.upsert({
      where: { roomNumber: room.roomNumber },
      update: {},
      create: { roomNumber: room.roomNumber, name: room.name, floor: room.floor, wing: room.wing,
        type: room.type, deskCount: room.deskCount, svgId,
        desks: room.type === 'DESK_ROOM'
          ? { create: Array.from({ length: room.deskCount }, (_, i) => ({ number: i + 1 })) }
          : undefined
      }
    })
    console.log(`  ✓ ${room.roomNumber}${room.name ? ` (${room.name})` : ''}`)
  }
